...
每个正式 Release 必须提供可直接安装的 Android APK 下载，并在发布前验证 APK 的版本号、签名和 SHA-256。是否重新构建 APK，应以上一个实际发布的正式 Release 为基线审计完整变更区间：

- 若变更可能影响移动端运行时或安装包内容，必须从本次 Release 提交重新构建生产签名 APK。触发范围包括但不限于 `apps/mobile/**`、移动端实际引用的共享包、移动端依赖或锁文件、Android 原生配置，以及影响 APK 生成结果的构建脚本。不得使用测试签名或仅供单一测试设备使用的 APK 作为正式 Release 附件。
- 若完整变更区间确认不影响移动端运行时或安装包内容，可复用最近一个已验证且兼容的正式 APK，无需无意义地重复编译。复用时必须保持 APK 的真实版本文件名与原始 SHA-256，并在 Release 正文及附件说明中明确标注来源 Release；不得重命名为当前版本或暗示它是本次新构建产物。
- 创建 Release 后必须检查附件列表，确认 APK 实际上传成功且可下载；只有 AAB、Actions Artifact 或外部商店链接不满足 APK 附件要求。
